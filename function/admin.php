...
---------------------------------------------------------                  
|                    CRUD PRODUK                        |
|   Berisi fungsi - fungsi untuk mengelola data barang  |    
---------------------------------------------------------
*/
// Membuat Kode barang
...
function home()
{
    ...
    $trans = mysqli_query($con, "SELECT * FROM penjualan JOIN status ON status.id_status=transaksi.id_status WHERE transaksi.id_status < 2 ORDER BY id DESC LIMIT 7 ");
    $tran = [];
...
function tambahProduk($post)
{
    ...
    if (isset(_validation($data)['pesan'])) {
        header('location:admin/tambahProduk.php/?pesan=' . _validation($data)['pesan']);
        die;
    }
    ...
    if (mysqli_affected_rows($konek)) {
        ...
        return header('location:admin/produk.php/?pesan=Data berhasil ditambahkan');
    } else {
        echo "errror";
        die;
    }
...
function ubahProduk($id, $post)
{
    ...
    if (isset(_validation($data)['pesan'])) {
        ...
        return header('location:admin/detailProduk.php/?id=' . $id);
    }
    _hapusSession();
    ...
    if ($sql > 0) {
        return header('location:admin/produk.php/?pesan=Data berhasil Diubah');
    }
...
function verifiTransaksi($id)
{
    ...
    return header('location:admin/transaksi.php');
}
...
function hapusPengguna($id)
{
    ...
    return header('location:admin/pengguna.php');
}
